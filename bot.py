...
class BaleBotClient:
    ...
    async def send_message(self, chat_id, text, components=None, reply_to_message_id=None):
        payload = {"chat_id": chat_id, "text": strip_markup(text)}
        if components:
    ...
    async def edit_message(self, chat_id, message_id, text, components=None):
        payload = {"chat_id": chat_id, "message_id": message_id, "text": strip_markup(text)}
        if components:
            payload["reply_markup"] = components
...
async def main():
    ...
    try:
        while True:
            updates = await BALE_BOT.get_updates(offset=offset, timeout=30)
